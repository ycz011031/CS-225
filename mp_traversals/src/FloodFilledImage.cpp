...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  int counter = 0;
  PNG the_image = png_;
  for (const Point &point :(*imagetraversal_)){
    if(counter % frameInterval == 0){
      animation.addFrame(the_image);
    }
    HSLAPixel& temp_pix = the_image.getPixel(point.x,point.y);
    temp_pix = colorpicker_->getColor(point.x,point.y);
    ++counter;
  }
  animation.addFrame(the_image);
  return animation;
