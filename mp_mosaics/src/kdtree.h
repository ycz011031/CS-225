...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode * nodeconnector(vector<Point<Dim>>& newPoints , int curDim);
    void  _delete(KDTreeNode* node);
    KDTreeNode * findNearestNeighbor(KDTreeNode* node, const Point<Dim>& query, int dimension) const;
};
